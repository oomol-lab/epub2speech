...
Examples:
  ...
  %(prog)s input.epub output.m4b --voice zh-CN-XiaoxiaoNeural --workspace /tmp/workspace
    ...
    parser.add_argument(
        "--voice", type=str, default="zh-CN-XiaoxiaoNeural", help="TTS voice name (default: zh-CN-XiaoxiaoNeural)"
    )
    ...
    if not args.azure_key or not args.azure_region:
        ...
        print(
            "Please provide via --azure-key and --azure-region parameters, or set AZURE_SPEECH_KEY and AZURE_SPEECH_REGION environment variables",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    try:
        ...
        tts_provider = AzureTextToSpeech(subscription_key=args.azure_key, region=args.azure_region)
